...
fn get_bid(contract: &Value, active_exchanges: &[&str]) -> f64 {
    ...
    if let Some(v) = contract["bestBid"].as_f64() {
        return v;
    }
...
pub fn compute_gamma_rows(
    options_data: &Value,
    spot: f64,
    active_exchanges: &[&str],
) -> Vec<Value> {
    if options_data.is_null() || spot <= 0.0 {
        return vec![];
    }
    let empty = vec![];
    ...
    for expiry in &expirations {
        ...
        if let (Some(ac), Some(ap)) = (atm_call, atm_put) {
            ...
            if call_ask > 0.0 && put_ask > 0.0 {
                let gamma =
                    ac["gamma"].as_f64().unwrap_or(0.0) + ap["gamma"].as_f64().unwrap_or(0.0);
                let theta =
                    ac["theta"].as_f64().unwrap_or(0.0) + ap["theta"].as_f64().unwrap_or(0.0);
        ...
        let otm_puts: Vec<&Value> = puts
            ...
            .collect();
        let has_delta = otm_calls
            .iter()
            .any(|c| c["delta"].as_f64().map(|d| d != 0.0).unwrap_or(false));
        ...
        for call in &otm_calls {
            for put in &otm_puts {
                if has_delta {
                    ...
                    if net_delta.abs() > DELTA_TOL {
                        continue;
                    }
                }
                let gamma =
                    call["gamma"].as_f64().unwrap_or(0.0) + put["gamma"].as_f64().unwrap_or(0.0);
                let theta =
                    call["theta"].as_f64().unwrap_or(0.0) + put["theta"].as_f64().unwrap_or(0.0);
                let be = match calc_break_even(theta, gamma) {
                    Some(b) => b,
                    None => continue,
                };
        ...
        if let Some((row, _, _)) = best_long.as_ref() {
    ...
    for expiry in &expirations {
        ...
        let make_vega_row = |type_: &str,
         ...
         -> Option<Value> {
            ...
            let mark_iv = (call["markVol"].as_f64().unwrap_or(0.0)
                + put["markVol"].as_f64().unwrap_or(0.0))
                / 2.0;
        ...
        if let (Some(ac), Some(ap)) = (atm_call, atm_put) {
            ...
            let put_ask = get_ask(ap, active_exchanges);
            if call_ask > 0.0 && put_ask > 0.0 {
                if let Some(row) = make_vega_row(
                    "straddle",
                    ac,
                    ap,
                    call_ask,
                    put_ask,
                    get_bid(ac, active_exchanges),
                    get_bid(ap, active_exchanges),
                ) {
                    results.push(row);
                }
            }
        ...
        for call in &otm_calls {
            for put in &otm_puts {
                if has_delta {
                    let net = call["delta"].as_f64().unwrap_or(0.0)
                        + put["delta"].as_f64().unwrap_or(0.0);
                    if net.abs() > DELTA_TOL {
                        continue;
                    }
                }
                let call_ask = get_ask(call, active_exchanges);
                ...
                let row = match make_vega_row(
                    "strangle",
                    call,
                    put,
                    call_ask,
                    put_ask,
                    get_bid(call, active_exchanges),
                    get_bid(put, active_exchanges),
                ) {
                    Some(r) => r,
                    None => continue,
                };
