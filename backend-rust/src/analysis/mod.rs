...
fn resolve_bid(contract: &Value) -> f64 {
    ...
    if let Some(prices) = contract["prices"].as_object() {
        let bids: Vec<f64> = prices
            .values()
            .filter_map(|p| p["bid"].as_f64())
            .filter(|&v| v > 0.0)
            .collect();
        if !bids.is_empty() {
            return bids.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        }
    }
    0.0
}
...
fn resolve_ask(contract: &Value) -> f64 {
    ...
    if let Some(prices) = contract["prices"].as_object() {
        let asks: Vec<f64> = prices
            .values()
            .filter_map(|p| p["ask"].as_f64())
            .filter(|&v| v > 0.0)
            .collect();
        if !asks.is_empty() {
            return asks.iter().cloned().fold(f64::INFINITY, f64::min);
        }
    }
    0.0
}
...
fn compute_atm_bbo_spread(expirations: &[String], response: &Value, spot: f64) -> Vec<Value> {
    ...
    for exp in expirations {
        ...
        let spreads: Vec<(f64, f64)> = [atm_call, atm_put]
            .iter()
            .filter_map(|opt| *opt)
            .filter_map(|contract| {
...
fn format_unique_bucket_labels(centers: &[f64]) -> Vec<String> {
    let mut decimals: Vec<usize> = centers
        .iter()
        .map(|&c| {
            if ((c.exp() - 1.0) * 100.0).abs() >= 2.0 {
                0
            } else {
                1
            }
    ...
    for _ in 0..6 {
        ...
        let dup_indices: Vec<usize> = seen
            .into_values()
        ...
        let mut bucket_map: std::collections::HashMap<i64, (f64, usize, f64, f64, bool, bool)> =
            centers
                ...
                .map(|&c| {
                    (
                        (c * 1e4).round() as i64,
                        (0.0, 0, f64::INFINITY, f64::NEG_INFINITY, false, false),
                    )
                })
                .collect();
...
pub fn compute_analysis(response: &Value, spot: f64) -> Option<Value> {
    ...
    for exp in &expirations {
        ...
        let atm_call = calls.iter().min_by(|a, b| {
            let da = (a["strike"].as_f64().unwrap_or(0.0) - spot).abs();
            let db = (b["strike"].as_f64().unwrap_or(0.0) - spot).abs();
            da.partial_cmp(&db).unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    let svi_fits: serde_json::Map<String, Value> = expirations
        .iter()
        .map(|exp| {
            let v = fits_internal
                .get(exp)
