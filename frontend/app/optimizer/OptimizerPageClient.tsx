...
export default function OptimizerPageClient() {
  ...
  const handleRun = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(apiPath(`optimizer/${coin}`), {
        method: 'POST',
        ...
        <div className="flex gap-4 items-start">
          <div className="w-72 shrink-0">
            <TargetInputs
              ...
              exchanges={exchanges}
              onExchangeChange={(exchange) => setExchanges((prev) =>
                prev.includes(exchange)
                  ? prev.length > 1 ? prev.filter((value) => value !== exchange) : prev
                  : [...prev, exchange]
              )}
              onRun={handleRun}
            />
