...
const PORTFOLIO_API_URL: Record<PortfolioExchange, string> = {
  okx: 'http://localhost:3500/api/portfolio/okx',
  bybit: 'http://localhost:3500/api/portfolio/bybit',
}
...
function buildOptionsUrl(exchange: Exchange, coin: 'BTC' | 'ETH' | 'SOL') {
  return exchange === 'okx'
    ? `http://localhost:3500/api/okx/options/${OKX_FAMILY_MAP[coin]}`
    : exchange === 'combined'
      ? `http://localhost:3500/api/combined/options/${coin}`
      : `http://localhost:3500/api/bybit/snapshot/${coin}`
}
